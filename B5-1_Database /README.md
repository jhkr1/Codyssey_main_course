...
- [ ] MySQL 컨테이너를 Docker로 실행할 수 있다.
- [ ] `schema.sql`로 데이터베이스와 테이블을 만들 수 있다.
- [ ] `sample_data.sql`로 샘플 데이터를 입력할 수 있다.
- [ ] 5개 테이블의 역할을 이해했다.
- [ ] PK와 FK의 차이를 이해했다.
- [ ] FK가 N쪽 테이블에 들어가는 이유를 이해했다.
- [ ] `order_detail.unit_price`를 따로 저장한 이유를 이해했다.
- [ ] `INNER JOIN`과 `LEFT JOIN`의 차이를 이해했다.
- [ ] `GROUP BY`로 집계 결과를 만드는 과정을 이해했다.
- [ ] 인덱스의 장점과 비용을 함께 이해했다.
- [ ] MySQL과 PostgreSQL의 기본 차이를 이해했다.
