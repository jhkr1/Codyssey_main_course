...
그러면 `user:1`은 3번 버킷, `user:2`는 4번 버킷에 저장된다.
...
1  ->  [user:3, Charlie]
2  ->  empty
3  ->  [user:1, Alice] <-> [cart:9, Book] <-> [token:7, live]
4  ->  [user:2, Bob]
5  ->  empty
6  ->  empty
7  ->  empty
...
3번 버킷에서는 충돌이 일어났다. 하지만 데이터를 덮어쓰지 않고, 연결 리스트에 차례로 붙여 모두 보관한다.

`GET cart:9`가 들어오면 전체 버킷을 보지 않는다. 3번 버킷으로 간 뒤, 그 안에서만 찾는다.
...
힙은 트리처럼 생각할 수 있지만, 실제 구현은 배열로 한다.
