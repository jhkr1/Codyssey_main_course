...
from budget_app.validators import parse_amount, parse_date, parse_month, parse_tags, parse_type
...
class TransactionService:
    ...
    def search(self, criteria: SearchCriteria) -> list[Transaction]:
        if criteria.date_from:
            parse_date(criteria.date_from)
        if criteria.date_to:
            parse_date(criteria.date_to)
        if criteria.tx_type:
            parse_type(criteria.tx_type)
        if criteria.category:
            self._validated_category(criteria.category)
    ...
    def update(self, transaction_id: str, changes: dict[str, Optional[str]]) -> bool:
        if changes.get("date"):
            parse_date(str(changes["date"]))
        if changes.get("type"):
            parse_type(str(changes["type"]))
        if changes.get("category"):
            self._validated_category(str(changes["category"]))
        ...
        def updater(transaction: Transaction) -> Transaction:
            return replace(
                transaction,
                date=str(changes["date"]) if changes.get("date") else transaction.date,
                type=str(changes["type"]) if changes.get("type") else transaction.type,
                category=str(changes["category"]) if changes.get("category") else transaction.category,
                ...
                tags=parse_tags(str(changes["tags"])) if changes.get("tags") is not None else transaction.tags,
            )

        return self.transactions.replace(transaction_id, updater)
    ...
    def export_csv(
        ...
        with Path(out_path).open("w", encoding="utf-8", newline="") as file:
            ...
            writer.writeheader()
            for transaction in sorted(rows, key=lambda item: (item.date, item.id), reverse=True):
                writer.writerow(
                    {
